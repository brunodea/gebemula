...
impl Timer {
    pub fn update(&mut self, cycles: u32, memory: &mut mem::Memory) {
        let internal_counter = memory.read_byte(consts::TIMER_INTERNAL_COUNTER_ADDR);
        let div = memory.read_byte(consts::DIV_REGISTER_ADDR);
        ...
        memory.write_byte(consts::TIMER_INTERNAL_COUNTER_ADDR, internal_timer as u8);
        memory.write_byte(consts::DIV_REGISTER_ADDR, (internal_timer >> 8) as u8);

        if self.timer_overflow {
            self.timer_overflow = false;
            let tima = memory.read_byte(consts::TMA_REGISTER_ADDR);
