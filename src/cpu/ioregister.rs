...
pub fn update_stat_reg_coincidence_flag(memory: &mut mem::Memory) {
    let coincidence_flag = if memory.read_byte(consts::LY_REGISTER_ADDR) ==
                                  memory.read_byte(consts::LYC_REGISTER_ADDR) {
        0b100
    } else {
        0b000
    };
    let new_stat = (memory.read_byte(consts::STAT_REGISTER_ADDR) & 0b1111_1011) |
                       coincidence_flag;
    memory.write_byte(consts::STAT_REGISTER_ADDR, new_stat);
}

pub fn update_stat_reg_mode_flag(mode_flag: u8, memory: &mut mem::Memory) {
    let new_stat = (memory.read_byte(consts::STAT_REGISTER_ADDR) & 0b1111_1100) |
                       (mode_flag & 0b11);
    memory.write_byte(consts::STAT_REGISTER_ADDR, new_stat);
}

pub fn lcdc_stat_interrupt(memory: &mut mem::Memory) {
    let reg = memory.read_byte(consts::STAT_REGISTER_ADDR);
...
impl LCDCRegister {
    ...
    pub fn disable_lcd(memory: &mut mem::Memory) {
        let val = memory.read_byte(consts::LCDC_REGISTER_ADDR);
...
pub fn joypad_set_buttons(new_buttons: u8, memory: &mut mem::Memory) {
    let mut buttons = memory.read_byte(consts::JOYPAD_REGISTER_ADDR);
