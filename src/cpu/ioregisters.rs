...
impl Interrupt {
    ...
    pub fn is_requested(interrupt: Interrupt, memory: &mem::Memory) -> bool {
        Interrupt::is_set_bit(Interrupt::bit(interrupt), IF_REGISTER_ADDR, memory)
    }

    pub fn request(interrupt: Interrupt, memory: &mut mem::Memory) {
        Interrupt::set_bit(Interrupt::bit(interrupt), IF_REGISTER_ADDR, memory);
    }

    pub fn remove_request(interrupt: Interrupt, memory: &mut mem::Memory) {
        Interrupt::unset_bit(Interrupt::bit(interrupt), IF_REGISTER_ADDR, memory);
    }

    pub fn next_request(memory: &mem::Memory) -> Option<Interrupt> {
        //order of priority
        for bit in 0..5 {
            let interrupt: Interrupt = Interrupt::from_bit(bit);
            if Interrupt::is_requested(interrupt, memory) {
    ...
    pub fn is_enabled(interrupt: Interrupt, memory: &mem::Memory) -> bool {
        Interrupt::is_set_bit(Interrupt::bit(interrupt), IE_REGISTER_ADDR, memory)
    }

    pub fn enable(interrupt: Interrupt, memory: &mut mem::Memory) {
        Interrupt::set_bit(Interrupt::bit(interrupt), IE_REGISTER_ADDR, memory);
    }

    pub fn disable(interrupt: Interrupt, memory: &mut mem::Memory) {
        Interrupt::unset_bit(Interrupt::bit(interrupt), IE_REGISTER_ADDR, memory);
    }
